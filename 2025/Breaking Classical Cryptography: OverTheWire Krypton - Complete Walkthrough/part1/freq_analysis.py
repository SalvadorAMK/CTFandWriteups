#!/usr/bin/python3
# ==============================================================================
# Developed by Cybersecurity Engineer: Abdelrahman Mohamed
# This code is part of the Medium story:
# "Breaking Classical Cryptography: OverTheWire Krypton — Complete Walkthrough (Part 1)"
# https://medium.com/@abdelrhmanmohamed.sec
# https://www.linkedin.com/in/abdelrhman-mohamed-sec
# PURPOSE: Performs frequency analysis on a ciphertext file.
#          Counts how many times each character (or N-gram group) appears.
#          Used to identify the most frequent letters and match them to known
#          English letter frequencies (E, T, A, O, I, N, S...) or find
#          repeating trigrams like "THE" to determine Vigenere key length.
#
# Usage:
#   python3 freq_analysis.py <filename> <groupsize>
#   Example: python3 freq_analysis.py found1 1      # single character frequency
#   Example: python3 freq_analysis.py found1 3      # trigram frequency
# ==============================================================================

import sys

if __name__=="__main__":

	char_table = {}
	char_total = 0
	groupsize = 0

	#Check Argument Counts
	if len(sys.argv) != 3:
		print("Usage: python3 freq_analysis.py filename groupsize")
		exit(0)
	else:

		try:
			groupsize = int(sys.argv[2])
		except:
			print("groupsize must be an int")
			exit(0)

		# Try to Open the specified file
		try:
			with open(sys.argv[1]) as fh:
				lines = fh.readlines()
		except:
			print("No file named '" + sys.argv[1] + "'")
			exit(0)

		for line in lines:
			line = line.replace(" ", "")
			line = line.replace("\n", "")
			for i in range(len(line) - groupsize):
				group = line[i:i+groupsize]
				if group in char_table:
					char_table[group] += 1
				else:
					char_table[group] = 1

		char_table = sorted(char_table.items(), key=lambda x: x[1], reverse=True)

		for char in char_table:
			print(char[0] + ":\t" + str(char[1]))
